...
func (self *Lexer) NextToken() (line,kind int, token string){
	...
	case ':':
		if self.test("::"){
			...
			self.next(2);return self.line,TOKEN_SEP_COLON,":"
		}		
	case '/':
		if self.test("//"){
	...
	if c == '_' || isLetter(c){
		...
		self.error("unexpected symbol near %c", c)
	}
	return 
}
...
func (self *Lexer) escape(str string) string{
	...
	for len(str)>0 {
		...
		self.error("invalid escapee sequence near '\\%c'",str[1])
	}
	return buf.String()
}
...
func (self *Lexer) NextTokenOfKind(kind int) (line int, token string){
	...
	if kind != _kind{
		self.error("sytax error near '%s'",token)
	}
	return line, token
}
